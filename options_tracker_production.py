...
class Config:
    ...
    WS_SUBSCRIPTION_CHUNK_SIZE = int(os.getenv('WS_SUBSCRIPTION_CHUNK_SIZE', '50'))
    WS_SUBSCRIPTION_DELAY = float(os.getenv('WS_SUBSCRIPTION_DELAY', '0.2'))
    ...
    WRITE_QUEUE_SIZE = int(os.getenv('WRITE_QUEUE_SIZE', '10000'))
...
class ProductionOptionsTracker:
    ...
    async def subscribe_with_retry(self, symbols: List[str]):
        ...
        while attempt < max_attempts and self.running:
            try:
                attempt += 1
                logger.info(f"WebSocket connection attempt {attempt}/{max_attempts}")
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    logger.info(f"Subscribed batch {i+1}/{len(chunks)}: {len(chunk)} symbols")
        ...
        for coin in Config.KNOWN_OPTION_ASSETS:
            ...
            while True:
                ...
                if data.get("retCode") != 0:
                    logger.warning(f"API returned error for {coin}: {data.get('retMsg')}")
                    break
            ...
            logger.info(f"Fetched {len([s for s in all_symbols if s.startswith(coin)])} {coin} options")
        ...
        if cache_path.exists():
            try:
                ...
                if datetime.now() < expires_at:
                    logger.info(f"Loaded {cache_data['count']} symbols from cache")
    ...
    async def run(self):
        ...
        logger.info(f"Tracking {len(symbols)} option symbols")
    ...
    def print_stats(self):
        ...
        logger.info(f"  Messages dropped: {self.metrics['messages_dropped']:,}")
        logger.info(f"  Redis writes: {self.metrics['redis_writes']:,}")
        logger.info(f"  Redis errors: {self.metrics['redis_errors']:,}")
        logger.info(f"  WebSocket reconnects: {self.metrics['ws_reconnects']:,}")
        logger.info(f"  Queue size: {len(self.write_queue)}/{Config.WRITE_QUEUE_SIZE}")
        
        if self.redis_client:
            try:
                info = self.redis_client.info('memory')
                logger.info(f"  Redis memory: {info.get('used_memory_human', 'N/A')}")
                logger.info(f"  Redis keys: {self.redis_client.dbsize():,}")
            except:
                pass
    ...
    async def cleanup(self):
        ...
        if self.write_queue:
            logger.info(f"Processing {len(self.write_queue)} remaining items...")
...
def main():
    ...
    if mode == 'fetch':
        ...
        logger.info("Fetching symbols only...")
        ...
        symbols = tracker.fetch_symbols_with_retry()
        if symbols:
            logger.info(f"✓ API connection successful ({len(symbols)} symbols)")
        else:
